...
> **最后更新**：2026-03-13
> **当前状态**：Phase 0–6 已完成，已完成统一存储布局重构 + 多流 SIMD 地址列解码
> **下一步**：Phase 7 查询流水线
...
vdb_storage
  ...
  ├── vdb_simd         (Phase 3: L2Sqr/BitUnpack/PrefixSum/Popcount/Transpose)
...
namespace vdb::simd {
    ...
    // 多流并行 exclusive prefix sum（交错布局）
    // 同时对 K 个独立流做前缀和，每条 SIMD lane 运行一个流
    // interleaved[j * 8 + k] = stream_k[j]
...
**AVX2 实现**：8×8 子块转置使用 `_mm256_unpacklo/hi_epi32`、`_mm256_unpacklo/hi_epi64`、`_mm256_permute2x128_si256`，处理 count%8 余量用标量路径。
...
namespace vdb::storage {
    struct AddressColumnLayout {
        uint32_t page_size;           // 页粒度（字节）
        uint8_t  bit_width;           // 集群共享位宽
        uint32_t block_granularity;   // 常规块记录数
        uint32_t fixed_packed_size;   // 常规块打包大小
        uint32_t last_packed_size;    // 尾块打包大小
        uint32_t num_address_blocks;  // 地址块数量
    };

    struct AddressBlock {
        uint32_t base_offset;         // 首条记录的页索引（uint32_t，可寻址 16TB@4KB）
        std::vector<uint8_t> packed;  // Bit-packed 页单位大小
    };
...
- 解码时乘回 page_size，最终 `AddressEntry::offset` 仍为 `uint64_t`
...
**性能**：8 流并行 vs 逐块串行约 50% 操作量减少（核心循环全程 uint32，无 32→64 展宽开销）。
...
Cluster Blocks (per cluster):
  ...
  Address blocks packed 数据（连续写入）
    block0.packed ... blockM-1.packed
  Mini-Trailer:
    page_size(u32) + bit_width(u8) + block_granularity(u32)
    + fixed_packed_size(u32) + last_packed_size(u32)
    + num_address_blocks(u32)
    + base_offsets[M] : u32[M]    ← v4 改为 uint32（v3 为 u64）
    + mini_trailer_size(u32) + block_magic(u32)
```

**版本历史**：v3 → v4（base_offsets 从 u64 收窄为 u32，节省 4×M 字节/集群）。读取器拒绝 v3 及更早版本。
...
Test project /home/zcq/VDB/VectorRetrival/build
      ...
      Start  1: test_types              ✅
      Start  2: test_status             ✅
      Start  3: test_aligned_alloc      ✅
      Start  4: test_schema             ✅
      Start  5: test_record             ✅
      Start  6: test_column_chunk       ✅
      Start  7: test_column_store       ✅
      Start  8: test_distance_l2        ✅
      Start  9: test_bit_unpack         ✅
      Start 10: test_prefix_sum         ✅  (含多流前缀和测试)
      Start 11: test_transpose          ✅  (NEW: 8×N 转置测试)
      Start 12: test_bitpack_codec      ✅
      Start 13: test_dict_codec         ✅
      Start 14: test_popcount           ✅
      Start 15: test_rabitq_rotation    ✅
      Start 16: test_rabitq_encoder     ✅
      Start 17: test_rabitq_estimator   ✅
      Start 18: test_address_column     ✅  (含多流解码一致性测试)
      Start 19: test_data_file          ✅
      Start 20: test_cluster_store      ✅  (含 v3 拒绝测试)
      Start 21: test_segment            ✅
      Start 22: test_conann             ✅
      Start 23: test_ivf_index          ✅
      Start 24: test_ivf_builder        ✅
      Start 25: bench_ivf_quality       ✅

100% tests passed, 0 tests failed
Total: ~400 individual test cases across 25 executables
```
...
| AddressColumn 解码 | 多流垂直并行 SIMD（K=8） | 8 流并行约 50% 操作减少 | **升级为多流管线** |
| base_offset 类型 | uint32_t 页索引 | 16TB@4KB 足够，使核心循环全程 uint32 | **从 u64 收窄为 u32** |
| .clu 文件版本 | v4 | base_offsets u64→u32 格式不兼容 | **v3→v4** |
| ClusterStore 加载 | 全量 codes 到内存 | RaBitQ 热路径优化 | 无变化 |
| Trailer 格式 | 文件末尾 mini-trailer 定位 | 自包含、无外部依赖 | 无变化 |
...
- **多流垂直并行 SIMD 地址列解码**（K=8 块并行，核心循环全程 uint32）
- **8×N 转置原语**（SoA↔交错布局，AVX2 8×8 子块转置）
- ~400 个单元测试 + Benchmark 工具
