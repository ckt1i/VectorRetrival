...
MS MARCO 的主表建议使用 `Q*=0.90`。`nprobe=256` 的原始主 sweep 曾出现尾延迟异常；当前主结果表已改用 triage full 的稳定重测值。
...
| MS MARCO | 256 | 0.9603 | 11.5918 | 20.3821 | 已用 triage full 稳定重测值替换原离群点 |
...
### 发现 9：MS MARCO 的 nprobe=256 原始主 sweep 曾是尾延迟离群点

**观察**: MS MARCO 的 nprobe=256 原始主 sweep 曾出现 recall@10 0.9603、平均 32.8081ms、p99 304.4682ms 的异常尾延迟；当前已改用 triage full 稳定重测值 recall@10 0.9603、平均 11.5918ms、p99 20.3821ms。

**解读**: 更像测量/系统噪声而非算法趋势。当前主实验表已不再使用旧离群值，改为 warmup 后的稳定重测结果。

**含义**: 论文正文主表和主曲线应采用替换后的稳定值；旧离群值只保留在实验过程记录里，不再作为结果证据。
