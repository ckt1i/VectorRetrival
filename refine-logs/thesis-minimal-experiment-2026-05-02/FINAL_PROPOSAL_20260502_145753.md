...
- COCO: `recall@10` + avg/p50/p95/p99 E2E latency。
- MS MARCO: 若 qrels 可用，报告 `MRR@10` / `nDCG@10`；否则先报告 ANN `recall@10`。
...
- Ablations:
  - disable SafeIn payload prefetch。
  - disable SafeOut pruning。
  - Uncertain eager payload。
- Metric: read amplification、bytes read、payload reads、E2E latency。

### Claim 3: 动态 fetch 调度降低 tail latency

- Experiment: dynamic submit vs fixed submit / serial fetch。
- Metric: p95/p99 latency、io wait、submit calls、flush counts。
...
3. Ablation：COCO 上完整做 triage 和 scheduling；MS MARCO 做关键 operating point。
4. Deep1M_synth：有时间则做第三数据集和 payload size sensitivity；否则用 COCO synthetic payload padding fallback。
