...
- [x] 9.3 更新 `refine-logs/EXPERIMENT_TRACKER_CN.md`（optional）
- [x] 9.4 更新 `refine-logs/BASELINE_PLAN_CN.md` 添加 "v5 实际结果" 小节（optional）
- [ ] 9.5 生成汇总图（optional）

## 10. 清理与归档

- [x] 10.1 删除/移动临时文件（Deep8M build 过程中的中间文件、失败的 diskann 尝试）
- [x] 10.2 在 `/home/zcq/VDB/baselines/README.md` 简短记录 v5 新增的脚本和数据路径
- [x] 10.3 验证所有脚本的 shebang 使用 `#!/usr/bin/env python3`，不直接硬编码 labnew 路径（脚本内注释说明运行环境要求）
