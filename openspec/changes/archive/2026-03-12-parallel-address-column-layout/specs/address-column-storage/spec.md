...
且结尾部分存储一个 `base_offsets[]` 数组，每个地址块一个条目
...
### 需求：平行常规块解码
读取器必须支持将多个常规地址块作为一个 SIMD oriented 批处理同时解码。
...
#### 场景：读取器接受新格式
给定一个以版本 `3` 写入的 `.clu` 文件
当读取器打开文件
那么它解析新的集群范围的地址结尾部分格式

#### 场景：读取器在没有兼容性路径的情况下拒绝旧格式
给定一个以版本 `2` 写入的 `.clu` 文件
当当前读取器打开文件
那么它返回不支持格式或损坏状态，而不是静默错误解析文件
