...
总结：需要你决定的关键权衡
#	决策	选项	推荐	决定
1	O_DIRECT	初期 buffered / 初期 O_DIRECT	初期 buffered	✅ buffered
5	codes 加载策略	全量到内存 / 逐条 pread	全量到内存	✅ 全量
6	Probe 批量策略	全量 batch / 逐条 / mini-batch 64	mini-batch 64	✅ mini-batch 64
7	BACK task 关联	ReadTask 携带字段 / 外部 map	ReadTask 携带	✅ 不再需要（延迟 payload 无 BACK task）
8	payload 策略	全流式 (5种task) / 延迟payload (1种task)	需你判断	✅ 延迟 payload（方案 C）
9	Probe 粒度	逐 cluster / 逐 mini-batch	mini-batch	✅ mini-batch，≤1024 原子
3	PopBatch 混合	Qv 全取 / 按比例	Qv 全取	✅ Qv 全取
4	队列容量	无界 / 有界 256	有界 256	✅ 有界 256
13	AsyncReader 抽象	薄封装 / 厚封装	薄封装	✅ 薄封装

---

## 后续实现级权衡（2026-03-16 追加）

### 权衡 14：liburing 编译依赖
- 方案 A: 条件编译 (#ifdef VDB_HAS_IO_URING)
- 方案 B: liburing 为必须依赖

**决定：方案 B 变体** — CMake `find_library(uring REQUIRED)` 构建时必须有 liburing，找不到则报错。
运行时 io_uring 初始化失败（内核不支持等）则打印警告并自动降级到 PreadFallbackReader。

### 权衡 15：Payload 最终读取方式
- 方案 A: 同步 pread 逐条读（简单但串行）
- 方案 B: AsyncReader 批量异步读
- 方案 C: 提供双 API（有/无 payload）

**决定：方案 B** — Finalize 排序后，对最终 top-k 结果通过 AsyncReader 批量发 I/O 读 payload。
堆内存储 (vec_id, dist, AddressEntry)，AddressEntry 用于定位 payload 在 .dat 文件中的位置。

### 权衡 16：精排距离计算接口
- 方案 A: 新增独立 `L2Sqr(a, b, dim)` 函数
- 方案 B: 在 RaBitQEstimator 上加 ComputeL2
- 方案 C: Rerank 时构造临时 RaBitQCode 再走估算路径

**决定：方案 A** — 在 `include/vdb/common/distance.h` 中新增独立 `L2Sqr` 函数。
精排不涉及 RaBitQ 量化，语义上不应放在 estimator 中。后续可 SIMD 优化。

### 权衡 17：搜索过程错误处理
- 方案 A: I/O 失败跳过该记录（best-effort）
- 方案 B: 任何失败立即中止整个搜索
- 方案 C: 返回部分结果 + 错误信息

**决定：初期从简** — 遇到错误直接返回错误项，附带调试信息。不做 skip/retry。